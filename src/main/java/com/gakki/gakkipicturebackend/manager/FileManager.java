package com.gakki.gakkipicturebackend.manager;
 ...
 * 文件服务
 *
 * @deprecated 已经废弃，改为用upload包的模版方法进行优化
 */
@Slf4j
@Service
@Deprecated
...
public class FileManager {
    ...
    private void validPicture(MultipartFile multipartFile) {
        ...
        ThrowUtils.throwIf(fileSize > 2 * ONe_M, ErrorCode.PARAMS_ERROR, "上传图片大小不能超过2M");
        //2. 校验图片后缀
        String fileSuffix = FileUtil.getSuffix(multipartFile.getOriginalFilename());
        //允许上传的文件后缀列表（或集合）
        final List<String> ALLOW_FORMAT_LIST = Arrays.asList("jpg", "jpeg", "png", "gif", "webp");
        ThrowUtils.throwIf(!ALLOW_FORMAT_LIST.contains(fileSuffix), ErrorCode.PARAMS_ERROR, "上传图片格式错误");
    }
