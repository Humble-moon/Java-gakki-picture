package com.gakki.gakkipicturebackend.service.impl;
...
import com.gakki.gakkipicturebackend.api.aliyunai.model.CreateOutPaintingTaskRequest;
...
import com.gakki.gakkipicturebackend.manager.FileManager;
import com.gakki.gakkipicturebackend.manager.upload.FilePictureUpload;
import com.gakki.gakkipicturebackend.manager.upload.PictureUploadTemplate;
import com.gakki.gakkipicturebackend.manager.upload.UrlPictureUpload;
import com.gakki.gakkipicturebackend.mapper.PictureMapper;
...
import com.gakki.gakkipicturebackend.model.enums.SpaceLevelEnum;
...
import com.gakki.gakkipicturebackend.utils.ColorSimilarUtils;
import com.gakki.gakkipicturebackend.utils.ColorTransformUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (pictureId != null) {
            ...
            //仅本人或者管理员可以编辑图片，用户不能更新别人的图片
            if (!oldPicture.getUserId().equals(loginUser.getId()) && !userService.isAdmin(loginUser)) {
                throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
            }
            // 校验空间是否一致
            // 没传入spaceId，则复用原有图片的spaceId
            if (spaceId == null) {
                if (oldPicture.getSpaceId() != null) {
                    spaceId = oldPicture.getSpaceId();
                }
            } else {
                // 传入了spaceId，校验和原图片的spaceId是否一致
        ...
        picture.setSpaceId(spaceId); //指定spaceId
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        if (oldPicture.getReviewStatus().equals(reviewStatus)) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "图片审核状态重复");
        }
        //4.数据库操作
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        for (Element element : imgElementList) {
            String filreUrl = element.attr("src");
            if (StrUtil.isBlank(filreUrl)) {
                log.info("当前链接为空，已跳过：{}", filreUrl);
                continue;
            }
            //处理图片的地址，防止转义或者对象存储冲突的问题
            int questionMarkIndex = filreUrl.indexOf("?");
            if (questionMarkIndex > -1) {
                filreUrl = filreUrl.substring(0, questionMarkIndex);
            }
            //构造
            //上传图片
            PictureUploadRequest pictureUploadRequest = new PictureUploadRequest();
            pictureUploadRequest.setFileUrl(filreUrl);
            pictureUploadRequest.setPicName(namePrefix + (uploadCount + 1));
            try {
                PictureVO pictureVO = this.uploadPicture(filreUrl, pictureUploadRequest, loginUser);
    ...
    public void clearPictureFile(Picture oldPicture) {
        ...
        // 删除缩略图
        String thumbnailUrl = oldPicture.getUrl();
...
//        checkPictureAuth(loginUser, oldPicture);
    ...
    public void editPictureByBatch(PictureEditByBatchRequest pictureEditByBatchRequest, User loginUser) {
        ...
        filePictureWithNameRule(pictureList, nameRule);
